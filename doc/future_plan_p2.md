...
P2는 [`future_plan.md`](future_plan.md)의 장기 범위를 축소하지 않고, 5레벨 플레이에서 필요한 성장·전투·콘텐츠·제작 흐름을 하나의 실행 단계로 묶는다. 완료된 P1 기록은 [`completed/future_plan_p1.md`](completed/future_plan_p1.md)에 보관한다.
...
- 현재 작업 문서는 `future_plan_p2.md` 하나로 식별된다.
...
- [ ] SRD 종족 9개로 캐릭터를 생성할 수 있다.
- [ ] 9개 종족의 대표 런타임 특성이 resolver와 TurnLog에 반영된다.
- [ ] 12개 직업이 5레벨까지 성장 가능하다.
- [ ] ASI, Extra Attack, 5레벨 자원·주문 슬롯 진행이 동작한다.
- [ ] 실행 가능 주문이 정확히 50개다.
- [ ] 대표 몬스터가 누적 25종이다.
- [ ] aura, grapple, 지속 피해, swarm, 비행, 복합 AoE 대표 케이스가 동작한다.
- [ ] 7개 지형 효과와 6개 오브젝트 행동이 실행 가능하다.
- [ ] 시나리오 제작 UI에서 카탈로그 id를 선택해 노드를 구성할 수 있다.
- [ ] P2 검증 시나리오를 AI GM과 HUMAN GM에서 완주했다.
- [ ] 재접속 상태 복원과 GM private 정보 분리를 확인했다.
- [ ] `npm run build`와 관련 회귀 spec을 사용자가 실행해 통과를 확인했다.
