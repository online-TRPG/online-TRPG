...
### 기술 목표
- 로컬 LLM 환경에서도 동작 가능한 **engine-heavy, AI-light 아키텍처**를 설계한다.
...
- 로컬 LLM은 Ollama 기반 Gemma 4 계열 모델을 사용한다.
- 기준 추론 환경은 VRAM 8GB, RAM 64GB 노트북이다.
...
### 6.1 로컬 LLM 기반 추론 환경
외부 토큰 과금 API가 아닌 **로컬 LLM 실행 환경**을 기준으로 설계한다.

### 6.2 Structured Output
자연어를 바로 사용하지 않고, JSON Schema 기반 structured output으로 받아서 검증한다.
...
- DB 서버: 세션 / 캐릭터 / 로그 저장
- 문서 저장소: 룰북 / 시나리오 / 컴펜디엄
- LLM 서버: GPU가 있는 로컬 머신 또는 별도 추론 호스트

즉, **플랫폼 서버와 LLM 추론 서버를 분리할 수 있는 구조**를 기본으로 잡는다.
...
- `AI_CONTRACTS.md`: Ollama AI 역할별 입출력 계약
...
6. 로컬 환경을 고려해 engine-heavy, AI-light 구조를 유지한다.
