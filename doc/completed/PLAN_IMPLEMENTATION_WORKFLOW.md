# TRPG Platform with AI/Human GM - 프로젝트 작업 진행 순서
...
flowchart LR
    ...
    E --> F[6. 런타임 하네스 구축]
    F --> G[7. 평가 하네스 구축]
    G --> H[8. 전투 / NPC / NpcDialogue / Director 고도화]
    H --> I[9. 배포 / 시연 환경 구성]
```
...
- Google AI Studio / Gemini API 연결 환경 변수 구조 정의

### 완료 기준
- 프론트와 백엔드가 각각 실행 가능
- 공통 타입을 import할 수 있음
- 팀 전체가 동일한 코드 스타일 규칙 사용 가능
- 기준 LLM 제공자, 모델, timeout, rate limit fallback 정책이 문서화됨
...
- role별 prompt 관리 구조
- 기본 RAG 룰 검색 연결
- Gemma 4 계열 모델 timeout / retry / fallback / rate limit 정책
- AI 호출 추적 로그
...
- 기본 NPC 행동 선택 가능
- NPC 대사는 Actor와 분리된 NpcDialogue 역할에서 생성
- 일반 턴 응답이 Google AI Studio free tier 환경에서도 30초 이내를 목표로 동작
- LLM 실패 시에도 세션 진행이 중단되지 않음

### 이유
AI GM의 핵심 플레이 루프는 먼저 **입력 해석 → 엔진 처리 → 결과 서사화**가 닫혀야 한다.

사람 GM 세션은 이 단계 이전에도 플랫폼 코어와 엔진만으로 진행 가능해야 하며, AI 기능은 `사람 GM 대체`가 아니라 `AI GM 모드` 또는 `사람 GM 보조 기능`으로 붙인다.
...
## 10. 8단계 - 전투 / NPC / NpcDialogue / Director 고도화
...
- 프론트엔드 / 백엔드 배포 환경 구성
- DB 초기 데이터 시드 구성
- Google AI Studio API 키 및 제공자 설정 구조 확정
...
7. **그 다음 Actor, NpcDialogue, Director, 전투 고도화로 확장한다**
...
- Interpreter / Actor / NpcDialogue / Narrator / Director / Summarizer
...
- `PRODUCT_SCOPE.md`
- `RULESET_SRD5E_MVP.md`
- `DOMAIN_MODEL.md`
- `RUNTIME_SESSION_TURN_FLOW.md`
- `AI_RUNTIME_CONTRACTS.md`
- `QUALITY_MVP_ACCEPTANCE.md`
