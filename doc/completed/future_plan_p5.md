...
P5는 [`future_plan.md`](future_plan.md)의 장기 목표를 유지하면서 13~16레벨 고레벨 플레이와 여러 회차에 걸친 캠페인 운영, 공개 시나리오 탐색·fork 생태계를 실제 사용자 흐름으로 연결하는 단계다.
...
- 공개 시나리오 추천과 신고·비공개 정책을 함께 구현한다.
...
5. 반복 요청, 중복 일정 등록, 중복 downtime 완료를 idempotency 기준으로 차단한다.
...
## 8. P5-5. 공개 시나리오 탐색·추천·fork 생태계 MVP

현재 제품 결정: 공개 커뮤니티 기능을 과도하게 키우지 않기 위해 별점·리뷰 기능은 제거하고, 공개 탐색은 fork와 신고 moderation 중심으로 유지한다.

목표 사용자 경험:

- 사용자는 공개 시나리오를 레벨, 예상 시간, 태그, 최신순으로 탐색한다.
...
- 추천 MVP: 필터 적합도, fork 수, 최근 활동을 이용한 설명 가능한 정렬.
...
- 공개 revision 발행, 검색·fork·신고 흐름을 검증한다.
...
- 공개 탐색부터 세션 생성과 fork까지 사용자 흐름이 이어진다.
...
- [ ] 공개 검색·추천·fork·moderation 정책이 동작한다.
