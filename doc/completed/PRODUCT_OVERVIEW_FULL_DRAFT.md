...
본 프로젝트는 **캐릭터 시트, 주사위, 룰북/컴펜디엄, 세션 로그, 전투 상태 관리**를 포함한 **웹 기반 TRPG 게임 플랫폼**에 **AI Game Master 기능**과 **사람 GM 진행 기능**을 함께 결합한 서비스이다.

핵심 방향은 단순한 AI 채팅 서비스가 아니라, **실제 플레이가 가능한 TRPG 플랫폼 위에서 AI GM과 사람 GM이 같은 플랫폼 코어를 공유하도록 만드는 것**이다.

이 프로젝트는 전통적인 인간 GM 중심 TRPG를 그대로 복제하는 것이 아니라, **TRPG의 자유도와 규칙 기반 상호작용을 유지하면서 GM 역할을 디지털화하고, 필요 시 사람 GM이 직접 개입할 수 있는 하이브리드형 플랫폼**을 목표로 한다.

정렬 기준:

- 플랫폼 코어는 `AI GM`과 `사람 GM`이 함께 사용한다.
- 세션은 `gmMode = AI | HUMAN`을 가진다.
- 플레이어 화면은 공통으로 유지하고, 사람 GM만 운영 패널을 추가 사용한다.
- 일반 채팅과 행동 입력은 분리한다.
- 캐릭터는 세션 종속 데이터가 아니라 계정 소유 영속 데이터로 관리한다.
...
- 사람 GM이 직접 장면 진행, 자료 공개, NPC 발화, 전투 시작을 제어할 수 있는 **사람 GM 기능**을 구현한다.
- 사람 GM 없이도 시작 가능한 **솔로 플레이 또는 소규모 플레이 경험**과, 사람 GM이 진행하는 **멀티 플레이 세션 경험**을 함께 제공한다.
...
- 사람 GM 메시지 입력 및 메인 진행 문구 전송
- 사람 GM의 NPC 대사 입력
- 현재 시나리오 노드 수동 변경
- 자료/이미지/맵/핸드아웃 공개
- 사람 GM 세션에서의 전투 시작 및 진행 제어
- AI 기능을 사람 GM 보조 도구로 선택적으로 사용
...
AI GM 세션에서는 시스템이 기본 진행을 담당하고, 사람 GM 세션에서는 같은 플랫폼 위에서 GM이 직접 진행을 제어한다. 두 모드는 세션, 로그, 상태 엔진, 캐릭터, 자료 표시 구조를 공유한다.
...
Actor는 NPC 대사를 생성하지 않는다.

### NpcDialogue

- NPC 대사를 생성한다.
- Actor가 이미 고른 행동 후보나 공개 장면 맥락을 참고할 수 있지만, 행동을 선택하거나 상태를 변경하지 않는다.
...
- `PRODUCT_SCOPE.md`: 확정 제약, 라이선스 정책, MVP 범위
- `RULESET_SRD5E_MVP.md`: SRD 5e 기반 MVP 룰 범위
- `DOMAIN_MODEL.md`: 핵심 엔티티와 상태 변경 모델
- `RUNTIME_SESSION_TURN_FLOW.md`: 플레이어 입력 1회가 처리되는 서버 흐름
- `AI_RUNTIME_CONTRACTS.md`: Google AI Studio 기반 AI 역할별 입출력 계약
...
**본 프로젝트는 캐릭터 시트, 주사위, 룰북, 세션 관리가 가능한 웹 기반 TRPG 플랫폼 위에, 규칙 엔진과 상태 엔진을 공통 기반으로 두고 AI GM과 사람 GM을 모두 지원하는 하이브리드형 게임 플랫폼이다.**
