...
이 문서는 제품 범위, MVP 포함/제외 기준, 외부 의존성, 라이선스 제약을 정의한다.
...
- AI GM / 사람 GM 공통 제약
- SRD 기반 콘텐츠 정책
- Google AI Studio / Gemini API 사용 기준
- 온라인 멀티플레이 상태 처리 기준

## 핵심 요약

- MVP는 AI 채팅 앱이 아니라 여러 플레이어가 접속 가능한 TRPG 세션 완주를 목표로 한다.
- AI는 게임 상태를 확정하지 않고, 규칙 엔진과 상태 엔진이 최종 authority를 가진다.
- 모든 세션은 `gmMode = AI | HUMAN`을 가진다.
- 플레이어 화면은 공통으로 유지하고, 사람 GM은 별도 운영 패널을 추가 사용한다.
- 권리가 확인된 SRD/무료/오리지널 콘텐츠만 제품 데이터로 사용한다.
...
### Google AI Studio / Gemini API LLM 제약
...
- [../rules/ARCHITECTURE_RULES.md](../rules/ARCHITECTURE_RULES.md): 서버 권위성과 멀티플레이 상태 처리 원칙
- [../rules/AI_RUNTIME_RULES.md](../rules/AI_RUNTIME_RULES.md): AI 호출과 fallback 원칙
- [../rules/CONTENT_LICENSE_RULES.md](../rules/CONTENT_LICENSE_RULES.md): 콘텐츠/라이선스 원칙
- [../rules/PERMISSION_RULES.md](../rules/PERMISSION_RULES.md): GM 모드와 권한 분리 원칙
...
- `ERD_MVP_SESSION_SERVICE_MODEL.md`: 핵심 엔티티와 세션/시나리오 런타임 데이터 모델
...
- MVP 포함/제외 범위를 바꾸면 `QUALITY_MVP_ACCEPTANCE.md`의 완료 기준을 함께 갱신한다.
- AI 제공자나 모델 기준을 바꾸면 `AI_RUNTIME_CONTRACTS.md`와 환경변수 기준을 함께 확인한다.
- 콘텐츠 범위를 바꾸면 `CONTENT_LICENSE_RULES.md`의 허용/금지 기준과 충돌하지 않는지 확인한다.
