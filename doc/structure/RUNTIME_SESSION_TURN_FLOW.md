...
- AI GM 세션의 기본 턴 루프
- 사람 GM 세션의 상태 저장/브로드캐스트 합류 흐름
- 플레이어 액션 큐, AI 호출, 엔진 처리, 로그 저장, 실시간 이벤트
...
- LLM 호출이 실패해도 세션은 fallback으로 이어져야 한다.
...
- [../rules/ARCHITECTURE_RULES.md](../rules/ARCHITECTURE_RULES.md): 서버 권위성, 상태 이력, fallback 원칙
- [../rules/AI_RUNTIME_RULES.md](../rules/AI_RUNTIME_RULES.md): AI 호출과 검증 원칙
- [../rules/PERMISSION_RULES.md](../rules/PERMISSION_RULES.md): 사람 GM 세션 권한 원칙

## 관련 문서

- [AI_RUNTIME_CONTRACTS.md](AI_RUNTIME_CONTRACTS.md): AI 역할별 입출력 계약
- [ERD_MVP_SESSION_SERVICE_MODEL.md](ERD_MVP_SESSION_SERVICE_MODEL.md): 런타임 DB 역할
- [trpg_main_command_mvp_flow_with_categories.md](trpg_main_command_mvp_flow_with_categories.md): 메인 커맨드 처리 구조

## 변경 시 주의사항

- 턴 처리 순서를 바꾸면 `PlayerAction`, `TurnLog`, `StateDiff`, 실시간 이벤트 저장 순서를 함께 확인한다.
- AI 호출 정책을 바꾸면 timeout/fallback 기준과 `AI_RUNTIME_CONTRACTS.md`를 함께 갱신한다.
