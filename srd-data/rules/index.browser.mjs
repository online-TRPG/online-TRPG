import classes from '../generated/srd/classes.json' with { type: 'json' };

// Keep behavior in sync with index.cjs; verify:rule-data-sync compares entrypoint results.
...
export function normalizeSrdCharacterClassKey(className) {
...
export function normalizeSrdCharacterLevel(level) {
...
export function getSrdClassDefinition(className, options = {}) {
...
export function getSrdClassSpellcastingProgression(className, level, options = {}) {
...
export function getCantripsKnownLimit(className, level, options = {}) {
  return getSrdClassSpellcastingProgression(className, level, options)?.cantripsKnown ?? null;
}

export function getKnownSpellsLimit(className, level, options = {}) {
  return getSrdClassSpellcastingProgression(className, level, options)?.spellsKnown ?? null;
}

export function resolveSubclassChoiceLevel(className, options = {}) {
  const classDefinition = getSrdClassDefinition(className, options);
...
export function resolveSpellcastingAbility(className, options = {}) {
  const classDefinition = getSrdClassDefinition(className, options);
...
export function resolvePreparedSpellAbility(className, options = {}) {
  const classKey = normalizeSrdCharacterClassKey(className);
...
export function resolvePreparedSpellLimit(input) {
...
export function resolveWizardSpellbookSpellCount(level) {
  const normalizedLevel = normalizeSrdCharacterLevel(level);
  return WIZARD_STARTING_SPELLBOOK_SPELL_COUNT +
    (normalizedLevel - 1) * WIZARD_SPELLBOOK_SPELLS_PER_LEVEL;
}

export function resolveMaximumCastableSpellLevel(className, level, options = {}) {
...
export function resolveSpellSlotLimit(className, level, slotLevel, options = {}) {
...
export function resolveAbilityScoreImprovementLevels(className) {
  const classKey = normalizeSrdCharacterClassKey(className);
  return Array.from(new Set([...STANDARD_ASI_LEVELS, ...(CLASS_ASI_LEVELS[classKey] ?? [])]))
    .sort((left, right) => left - right);
}

export function resolveAvailableAbilityScoreImprovementLevels(className, level) {
  const normalizedLevel = normalizeSrdCharacterLevel(level);
  return resolveAbilityScoreImprovementLevels(className)
    .filter((asiLevel) => asiLevel <= normalizedLevel);
}

export function resolveCrossedAbilityScoreImprovementLevels(className, currentLevel, targetLevel) {
...
export function usesDynamicPreparedSpellPool(input) {
  const classKey = normalizeSrdCharacterClassKey(input?.classKey ?? input?.className);
...
export function resolveCharacterSpellSelectionRequirements(input) {
  const classKey = normalizeSrdCharacterClassKey(input?.classKey ?? input?.className);
  const level = normalizeSrdCharacterLevel(input?.level);
...
export function resolveKnownSpellDelta(input) {
  const classKey = normalizeSrdCharacterClassKey(input?.classKey ?? input?.className);
