...
- The active response schema is authoritative. Output an ID field only when that field exists in the schema. If a selected candidate is marked `selected=true` but its echo field is omitted, the backend restores the selected ID; do not add the omitted field.
...
- Treat `relatedEntities`, `relatedRules`, and `classFeatureCandidates` as reference context only.
- Use `sceneSummary`, `recentLogs`, and `targets` only to resolve known references from the current play log.
- Prefer stable IDs from retrieved context and `targets` over natural-language labels.
- Never create a `targetId`; when `action.targetId` exists in the active schema, copy it only from `targets`. A selected target is marked there once with `selected=true`; if the schema omits `action.targetId`, do not echo it.
...
- If the player clearly casts a retrieved spell as a normal combat/map action, set `action.type` to `MAP_CAST_SPELL`. Copy the spell ID into `action.spellId` and `mentionedSpellId` only when those fields exist in the active schema; otherwise the backend restores a selected spell ID.
...
- If the player clearly uses a class feature named in `classFeatureCandidates.sourceEntityIds`, set `action.type` to `MAP_USE_CLASS_FEATURE` and copy that feature ID into `action.featureId`.
...
- If the player clearly names a retrieved item, copy the matching ID into `mentionedItemId` only when that field exists in the active schema; otherwise the backend restores a selected item ID.
