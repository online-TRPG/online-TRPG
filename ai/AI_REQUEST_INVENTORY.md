...
- 트리거: `ACTION-001` 후 엔진이 `CheckRequest`, `DiceResult`, `StateDiff`를 확정한 뒤
- 역할: `Narrator`
- 중요도: MVP 필수
...
- 세션 모드별 허용/차단 정책 없음
- 사람 GM 세션 예외 정책 없음
- `Actor`, `Director`, `Summarizer` 하네스 없음
- `AI-001` ~ `AI-005` 대응 요청 DTO 없음
- `ACTION-001` 입력을 실제 `InterpreterInput` 구조로 조립하는 컨텍스트 빌더 없음
- Narrator에 `StateDiff`, `DiceResult`, `CheckRequest` 전체 구조 전달 없음
- role별 rule validator 없음
- fallback 템플릿 체계 없음
- `AiTrace` 저장 포맷 매핑 미완성
- 운영용 trace 조회/필터 DTO 없음
...
2. `Narrator`를 `DiceResult`, `StateDiff`, `noNewFacts` 검증까지 확장
3. `AI-001` 힌트용 `Director` 하네스 추가
4. `AI-004` 요약용 `Summarizer` 하네스 추가
5. `Actor` 하네스 추가
6. `AI-005` 대응 trace 조회 포맷 정리
