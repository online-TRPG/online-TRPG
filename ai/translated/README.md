...
런타임은 이 Markdown을 직접 읽지 않는다. 서버와 AI prompt는 repo root의 `srd-data/generated/srd/`에 있는 JSON/JSONL catalog를 읽는다.
...
진행표 성격의 파일은 과거 작업 흔적에 가깝다. 현재 완료 여부는 `srd-data/generated/srd/srd_qa_report.json`과 테스트 결과를 기준으로 본다.
...
4. 번역 Markdown을 고치면 `python -m app.srd.build --output-dir ..\srd-data\generated\srd`를 다시 실행한다.
...
python -m app.srd.build --output-dir ..\srd-data\generated\srd
