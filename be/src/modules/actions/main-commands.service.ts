...
type TransitionConditionRequirementType = "ALWAYS" | "CLUE_REVEALED" | "COMBAT_RESOLVED" | "NODE_VISITED" | "FLAG_SET" | "GM_APPROVAL";
...
export class MainCommandsService {
  ...
  constructor(
    ...
    private readonly mainCommandIntentHandlers: MainCommandIntentHandlersService = new MainCommandIntentHandlersService(),
  ) {}
  ...
  private createMainCommandIntentHandlersRuntime() {
    return {
      ...
      buildCheckOptions: this.buildCheckOptions.bind(this),
      buildDangerDetectionCheckOptions: this.buildDangerDetectionCheckOptions.bind(this),
      buildDeceptionCheckOptions: this.buildDeceptionCheckOptions.bind(this),
      buildInsightCheckOptions: this.buildInsightCheckOptions.bind(this),
      buildInterpreterPayload: this.buildInterpreterPayload.bind(this),
      buildIntimidationCheckOptions: this.buildIntimidationCheckOptions.bind(this),
      buildInvestigationCheckOptions: this.buildInvestigationCheckOptions.bind(this),
      buildItemExploreCheckOptions: this.buildItemExploreCheckOptions.bind(this),
      buildObjectInteractionCheckOptions: this.buildObjectInteractionCheckOptions.bind(this),
      buildPerceptionCheckOptions: this.buildPerceptionCheckOptions.bind(this),
      buildPersuasionCheckOptions: this.buildPersuasionCheckOptions.bind(this),
      buildSpecialMoveCheckOptions: this.buildSpecialMoveCheckOptions.bind(this),
      buildToolUseCheckOptions: this.buildToolUseCheckOptions.bind(this),
      canUseExplicitPlayerText: this.canUseExplicitPlayerText.bind(this),
      ...
      resolveEntity: this.resolveEntity.bind(this),
      resolveOwnedItemName: this.resolveOwnedItemName.bind(this),
      shouldRequireMainCommandCheck: this.shouldRequireMainCommandCheck.bind(this),
    };
  }
  ...
  async submitMainCommand(userId: string, sessionId: string, dto: SubmitMainCommandDto): Promise<MainCommandResponseDto> {
    ...
    const objectRevealResult =
      dto.intent === MainCommandIntent.INVESTIGATE_OBJECT && dto.mapPoint && response.status !== MainCommandStatus.CHECK_REQUIRED
    ...
    if (objectRevealResult.count > 0) {
      response = this.withRevealedObjectContents(response, objectRevealResult.revealedClues, objectRevealResult.revealedItems);
    }
    ...
    if (revealCount + objectRevealResult.count > 0) {
      ...
      this.realtimeEvents.emitSessionSnapshot(context.sessionId, await this.sessionsService.buildSnapshot(context.sessionId));
    }
    return response;
  }
  ...
  async resolveMainCommandCheck(userId: string, sessionId: string, dto: ResolveMainCommandCheckDto): Promise<MainCommandResponseDto> {
    ...
    const resultMessage = await this.buildMainCommandCheckResultMessageForOutcome(userId, session.id, mainCommandEffect, dto.outcome);
    let result = {
      status: dto.outcome === ActionOutcome.SUCCESS ? MainCommandStatus.RESOLVED : MainCommandStatus.MESSAGE,
    ...
    if (dto.outcome === ActionOutcome.SUCCESS && mainCommandEffect.intent === MainCommandIntent.INVESTIGATE_OBJECT && mainCommandEffect.mapPoint) {
    ...
    if (dto.outcome === ActionOutcome.SUCCESS && mainCommandEffect.intent === MainCommandIntent.OBSERVE_AREA) {
    ...
    if (dto.outcome === ActionOutcome.SUCCESS && mainCommandEffect.intent === MainCommandIntent.SPECIAL_MOVE && mainCommandEffect.mapPoint) {
      ...
      result = {
        ...
        message: moveResult.status === MainCommandStatus.RESOLVED ? `${result.message}\n\n${moveResult.message}` : moveResult.message,
      };
    ...
    if (dto.outcome === ActionOutcome.SUCCESS) {
      const revealedActionClues =
        ...
        !(mainCommandEffect.intent === MainCommandIntent.INVESTIGATE_OBJECT && mainCommandEffect.mapPoint)
  ...
  private async handleGeneralGmRequest(
    ...
    if (
      ...
      return {
        requestId,
        status: MainCommandStatus.MESSAGE,
        message: interpreter.parsed.clarificationQuestion ?? "어떤 행동이나 요청을 하려는지 조금 더 구체적으로 적어주세요.",
      };
    }
  ...
  private resolveTextFallbackMainCommandIntent(dto: SubmitMainCommandDto, actionType?: string | null): MainCommandIntent | null {
  ...
  private async handleHint(
    ...
    if (publicClues.length > 0 && eventHints.length === 0) {
      ...
      const unrevealedClues = publicClues.filter((clue) => !this.textEvidenceMatches(clue, revealedClueText));
  ...
  private async handleSceneTransition(
    ...
    if (satisfiedCandidates.length > 1) {
      return {
        requestId,
        status: MainCommandStatus.GM_APPROVAL_REQUIRED,
        message: `이동 가능한 분기가 여러 개입니다. 목적지를 지정해주세요. 가능한 목적지: ${satisfiedCandidates.map((item) => item.target.title).join(", ")}`,
      };
    }
    ...
    const blockedCandidates = evaluatedCandidates.filter((candidate) => candidate.conditionResult.missingTerms.length);
    return {
      requestId,
      status: MainCommandStatus.IMPOSSIBLE,
  ...
  private async handleRuleQuery(
    ...
    const matchingRules = this.loadRuleFragments().filter((fragment) => interpreter.parsed.requiredRuleCheckIds?.includes(fragment.id));
  ...
  private async buildTransitionEvidence(context: LoadedContext, recentLogs: string[]): Promise<TransitionEvidence> {
    ...
    const revealedClueText = this.normalizeTransitionConditionText(revealedClues.join(" "));
    const unrevealedClues = this.extractPublicClueSummaries(context.currentNodeCluesJson).filter((clue) => !this.textEvidenceMatches(clue, revealedClueText));
  ...
  private async loadRevealedClueState(sessionScenarioId: string): Promise<{ ids: string[]; summaries: string[] }> {
  ...
  private buildMainCommandCheckResultMessage(effect: MainCommandCheckEffect, outcome: ActionOutcome): string {
  ...
  private async buildSocialInformationSuccessMessage(userId: string, sessionId: string, effect: MainCommandCheckEffect): Promise<string> {
    ...
    return narration ? narration : this.buildMainCommandCheckResultMessage(effect, ActionOutcome.SUCCESS);
  }
  ...
  private buildFailedCheckNarration(effect: MainCommandCheckEffect, targetLabel: string, itemLabel: string, pointLabel: string): string {
    switch (effect.intent) {
      case MainCommandIntent.OBSERVE_AREA:
  ...
  private async persistResult(userId: string, context: LoadedContext, dto: SubmitMainCommandDto, response: MainCommandResponseDto) {
  ...
  private resolveEntity(dto: SubmitMainCommandDto, entities: VisibleSceneEntity[], preferredType?: MainCommandTargetType): VisibleSceneEntity | null {
    const filtered =
  ...
  private resolveEntityMentionedInText(playerText: string, entities: VisibleSceneEntity[]): VisibleSceneEntity | null {
  ...
  private canUseExplicitPlayerText(dto: SubmitMainCommandDto, options: { acceptsMapPoint?: boolean; acceptsTarget?: boolean } = {}): boolean {
    const text = dto.playerText.trim();
    if (!text) {
      return false;
    }
  ...
  private shouldRequireMainCommandCheck(action: { requiresRoll?: boolean | null }, dto: SubmitMainCommandDto, needsClarification: boolean): boolean {
    return (
  ...
  private evaluateTransitionConditionContract(contract: TransitionConditionCandidateContract, evidence: TransitionEvidence): TransitionConditionEvaluation {
